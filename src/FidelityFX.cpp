...
namespace FrameGen
{
	...
	void FidelityFX::Present(bool a_useFrameGeneration)
	{
		...
		// v0.25：NR 已移除（走 ReShade 外挂），FG 输入恒 colorOut。
		auto HUDLessColor = sc.colorOutWrapped ? sc.colorOutWrapped->resource.get() :
			(sc.swapChainBufferWrapped ? sc.swapChainBufferWrapped->resource.get() : nullptr);
