...
def dis(rva, size, label):
    ...
    for ins in md.disasm(data[off: off+size], base + rva):
        ...
        if m:
            ...
            extra = f"  ; -> {tgt:#x}"
        # mark calls into IAT / device vtable patterns
        print(f"{ins.address - base:#08x}: {ins.mnemonic} {ins.op_str}{extra}")
        if ins.address - base > rva + size - 0x10:
            break

# 0x1A880 continuation: from where the ctor field init ends (~0x1a997) to find device calls
dis(0x1A880, 0x280, "0x1A880 full (executor ctor continuation)")
